...
This is not the only way to solve the assignment, nor is it necessarily the best
or most efficient. However, it should be enough to help you progress to the next
phase.
...
class Mapper:
    ...
    def __init__(self):
        ...
        #create mapper object from MapDrawer class and mark starting point of turtlebot
        self.mapper = MapDrawer((35,15))

        # Robot linear movement state ('forwad', 'left', 'right', None)
        self.state = None

        # Direction to wander (left, right, None) (set by data from depth camera)
        self.wander_direction = None
        # Prevents the robot from changing tasks while executing its bump procedure
        #self.can_change_task = True
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # update map
            self.mapper.UpdateMapDisplay(self.map,
                                        self.WorldPositionToMap(self.position))
    ...
    def process_ekf(self, data):
        ...
        # continuously update the path traced by the turtlebot 
        if self.start_mapping:
            map_pos = self.WorldPositionToMap(self.position)
            self.map[int(map_pos[0] - 0.5):int(map_pos[0] + 0.5)+1,
                     int(map_pos[1] - 0.5):int(map_pos[1] + 0.5)+1] = 0
    ...
    def set_wander_direction(self, img):
        ...
        #average_filled_space = [np.nanmean(seg) for seg in img_segments]
        #rospy.loginfo({"avg":[round(each) for each in average_filled_space]})
        #rospy.loginfo({"count" : filled_space})
    ...
    def shutdown(self):
        ...
        self.mapper.SaveMap('solution_map.jpg', self.WorldPositionToMap(self.position))
